...
export class UiService {
  ...
  public newApp(date: Date, slot: number): void {
    ...
    if (this.userId === undefined) {
      ...
      this.http.post('http://localhost:8090/appointments', {
  ...
  public deleteAppt(id: number) {
    this.http.delete(`http://localhost:8090/appointments/${id}`)
    .pipe(take(1))
    .subscribe({
      next: () => {
        this.loadAppointments()
  ...
  private loadDoctorAppts(): void {
    this.http.get<Appointment[]>(`http://localhost:8090/appointments?doctorId=${this.userId}`)
      .pipe(take(1))
      .subscribe({
        next: appointments => {
  ...
  private loadPatientBookedAppts(): void {
    this.http.get<Appointment[]>(`http://localhost:8090/appointments?patientId=${this.userId}`)
      .pipe(take(1))
      .subscribe({
        next: appointments => {
  ...
  private loadAvailableAppts(): void {
    this.http.get<Appointment[]>('http://localhost:8090/appointments')
      .pipe(take(1))
      .subscribe({
        next: appointments => {
  ...
  public tryLogin(username: string, password: string) {
    this.http.get<User>(`http://localhost:8090/users?username=${username}&password=${password}`)
    .pipe(take(1))
    .subscribe({
      next: user => {
  ...
  public bookAppt(appointment: Appointment): void {
    this.http.put(`http://localhost:8090/appointments/${appointment.id}`, {
      ...appointment,
